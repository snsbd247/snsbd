  ...
  server: {
    handlers: {
      ...
      GET: async ({ request }) => {
        ...
        // Resellers see linked customers' services; others only their own.
        const { data: isReseller } = await supabaseAdmin.rpc("has_role", { _user_id: auth.userId, _role: "reseller" as any });
        let query = supabaseAdmin.from("services").select("id, name, status, customer_id, expiry_date, price, billing_cycle, created_at").limit(500);
        if (isReseller) {
