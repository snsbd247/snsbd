              ...
              {(invoices ?? []).map((i: any) => (
                <ClickableRow key={i.id} to="/invoices/$invoiceId" params={{ invoiceId: i.id }}>
                  ...
                  <TableCell className="text-right">
                    <StopClick>
                      <Button size="icon" variant="ghost" asChild className="hover:bg-sky-100 hover:text-sky-700 dark:hover:bg-sky-950/40"><Link to="/invoices/$invoiceId" params={{ invoiceId: i.id }}><Eye className="h-4 w-4" /></Link></Button>
                      {role === "admin" && (
